...
% id_country=1, plot the results for Germany, 2 for France, 3 for Italy.
...
id_country=1; % id for country 1=Germany, 2=France, 3=Italy


% load the fitting results for SIHM with w=w^{3}, time_changes=3-1=2
...
elseif id_country==3
    ...
    time_x=300;
    time3=366; % time (day) when alpha variant became dominant
    time_pr=333;
elseif id_country==4
       return
end
...
w3_pred=1.2*(1+xfmincon(9))-1;   %rho_alpha=1.2
...
%%%% threshold, u3 corresponds to Delta_u in the ms
if id_country==1
    u3=0.094; % peak 6300
elseif id_country==2
    u3=0.0644; % peak 11k
...
if id_country==1
    ...
    legend('DE dH data','dH fitting', 'dH prediction ({\it{e_p}}=0.05)','\Delta u_{{opt}}=0.09, peak < 6.5k','\Delta u_{opt}=0.15, peak < 5.5k')
...
elseif id_country==2
    ...
    legend('FR dH data','dH fitting','dH prediction ({\it{e_p}}=0.01)','\Delta u_{{opt}}=0.06, peak < 11k','\Delta u_{{opt}}=0.1, peak < 10k')
